...
// class Serializer {
// public:
//   template <typename T> static Json writePointer(T *instance) {
...
//     std::string type_name = json_context["$typeName"];
...
//   template <typename T> static T &read(const Json &json_context, T &instance) {
//     if constexpr (std::is_pointer<T>::value) {
